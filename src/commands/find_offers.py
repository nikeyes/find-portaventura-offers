...
class FindOffers:
    ...
    def __init__(self, hotel_prices_file: str, ticket_prices_file: str, date_ini: datetime, date_end: datetime, max_offers: int) -> None:
    ...
    def load_prices(self, hotel_prices_file):
        with open(hotel_prices_file, 'r') as json_file:
            ...
            if data != {}:
                return [
                    HotelPrice(date=d['date'], name=d['name'], rate=d['rate'], rate_old=d['rate_old'], discount=d['discount'])
                    for d in data.get('hotels_rate')
                ]
